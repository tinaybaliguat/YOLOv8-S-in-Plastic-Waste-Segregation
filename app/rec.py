...
class IntroVideoScreen(QWidget):
    def __init__(self, video_path, on_finished_callback):
        ...
        self.setWindowIcon(QIcon(resource_path("assets/icons/app_logo.png")))
        ...
        for name, sec in zip(names, seconds):
            ...
            if sec is None:
                btn.clicked.connect(self._end_video) 
            else:
...
class MplCanvas(FigureCanvas):
    ...
    def plot_data(self, data_dict):
        ...
        if data_dict:
            ...
            self.axes.set_yticks(np.arange(0, max(values) + 1, 1))
        else:
...
class WasteDetectionApp(QWidget):
    def __init__(self, splash=None):
        ...
        model_file_path = resource_path("models/best.pt")
    ...
    def on_model_loaded(self, model):
        ...
        self.populate_history_filter_combo()  
    ...
    def create_navigation_bar(self):
        ...
        nav_layout.setContentsMargins(5, 70, 5, 20) 
    ...
    def create_detection_view(self):
        ...
        self.conf_input.setFixedWidth(50)  
        ...
        stats_header_layout.addSpacing(8)
    ...
    def create_info_card(self, title_text, initial_value, card_object_name="statCard"):
        card_frame = QFrame()
        card_frame.setObjectName(card_object_name) 
        card_layout = QVBoxLayout(card_frame)
        card_layout.setContentsMargins(15, 15, 15, 15) 
        card_layout.setSpacing(5)  

        title_label = QLabel(title_text)
        title_label.setObjectName("statCardTitleLabel") 
        value_label = QLabel(initial_value)
        value_label.setObjectName("statCardValueLabel") 
    ...
    def create_analytics_view(self):
        ...
        self.chart_frame_container = QFrame() 
        self.chart_frame_container.setObjectName("chartFrame")  
        ...
        chart_title = QLabel("Detection Class Distribution") 
        ...
        main_layout.addWidget(self.chart_frame_container, 1)
    ...
    def create_history_view(self):
        ...
        # self.gallery_content_widget.setLayout(self.gallery_flow_layout) 
    ...
    def handle_accordion_toggle(self, checked, current_frame, current_button):
        ...
        if checked:
            current_frame.setMaximumHeight(current_frame.sizeHint().height() + 50) 
            current_frame.show()
    ...
    def toggle_webcam(self):
        checkpoint_path = resource_path("yolo/Yolov7_StrongSORT_OSNet/strong_sort/deep/checkpoint/osnet_x0_25_market1501.pt")
    ...
    def update_analytics_view(self):
        ...
        if labels and sum(sizes) > 0: 
            ...
            ax.axis('equal') 
            ax.set_title("Distribution of Detected Classes", fontsize=14, color='black') 
    ...
    def update_history_view(self, page=None):
        ...
        while self.gallery_flow_layout.count():
            item = self.gallery_flow_layout.takeAt(0) 
            widget = item.widget()
            if widget:
                widget.deleteLater()
    ...
    def show_history_details(self, history_record):
        ...
        details_text = f"<b>Record ID: {record_id}</b><br>"
        ...
        if history_record['detected_objects']:
            details_text += "<ul style='margin-left: 0px; padding-left: 15px;'>" 
        ...
        if pixmap_preview:
            msg_box.setIconPixmap(pixmap_preview) 
...
if __name__ == "__main__":
    ...
    def start_video():
        ...
        try:
            video_path = os.path.abspath(resource_path("assets/icons/plastic.mp4"))
